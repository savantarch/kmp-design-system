...
[![Kotlin](https://img.shields.io/badge/Kotlin-2.1.20-blue?logo=kotlin)](https://kotlinlang.org)
[![Compose Multiplatform](https://img.shields.io/badge/Compose_Multiplatform-1.8.2-purple?logo=jetpackcompose)](https://github.com/JetBrains/compose-multiplatform)
[![Maven Central](https://img.shields.io/badge/Maven_Central-v1.0.0--beta02-green?logo=apachemaven)](https://central.sonatype.com/)
...
To use the design system library in any KMP application, add the Gradle plugin and runtime
dependency.

### Add Plugin Repository

In your root `settings.gradle.kts`, configure `pluginManagement` to resolve the plugin:
...
In your shared UI module's `build.gradle.kts` (e.g. `:shared` or `:app-shared`), apply the plugin
and configure the `designSystem` extension block:

```kotlin
plugins {
    id("com.savantarch.designsystem") version "1.0.0"
}
...
Add the core design system library to your `commonMain` dependencies:

```kotlin
kotlin {
    sourceSets {
        commonMain.dependencies {
            implementation("com.savantarch:design-system:1.0.0")
        }
    }
}
```
